    ...
    public class WordAligner
    {
            ...
            ITokenizer? tokenizer = null)
        {
            ...
            foreach (var extraction in extractions)
            {
                ...
                int startIdx = FindSubSequence(sourceTokenStringsLower, extTokenStrings);

                if (startIdx != -1)
                {
                    int endIdx = startIdx + extTokenStrings.Count; 
        ...
        {
            int start = token.CharInterval.StartPos ?? 0;
            int end = token.CharInterval.EndPos ?? 0;
